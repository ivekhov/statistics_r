...
model_aux <- lm(data = h, Fertility ~ Examination)
summary(model_aux)
...
pred <- data.frame(Catholic = c(9.96), Agriculture=c(17), Examination=c(15))
predict(model, pred)
report <- summary(model)
report$residuals

predict(model_st, pred)


pred_h_st <- mutate_each(pred, "scale")
fert_mean <- 70.14
agr_mean  <- 50.66
exam_mean <- 16.49
...
StdDev(h$Catholic)
StdDev(h$Agriculture)
StdDev(h$Examination)

Mean(h$Catholic)
Mean(h$Agriculture)
Mean(h$Examination)

a <- (pred$Catholic - Mean(h$Catholic)) / StdDev(h$Catholic)
b <- (pred$Agriculture - Mean(h$Agriculture)) / StdDev(h$Agriculture)
c <- (pred$Examination - Mean(h$Examination)) / StdDev(h$Examination)


pred_scaled <- c(a, b, c)
pred_scaled <- data.frame(Catholic=a, 
                          Agriculture=b,
                          Examination=c)
...
predict(model_st, pred_scaled)
predict(model, pred)

predict(model, pred_scaled)
